...
describe("excel-export", () => {
  ...
  describe("exportToExcel", () => {
    ...
    beforeEach(() => {
      ...
      appendChildMock = vi.spyOn(document.body, "appendChild").mockImplementation(() => undefined as unknown as Node);
      removeChildMock = vi.spyOn(document.body, "removeChild").mockImplementation(() => undefined as unknown as Node);
    });
    ...
    it("should generate a workbook, create an object URL, and trigger a download", async () => {
      ...
      const createElementSpy = vi.spyOn(document, "createElement").mockImplementation((tagName) => {
        ...
        const el = (document.createElement as any).getMockImplementation()?.(tagName);
        return el;
      });

      await exportToExcel("Test_Export", "Schedule", data, columns as any);
